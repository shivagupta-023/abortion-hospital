...
export default function CredentialsSection() {
  ...
  return (
    <section
      ...
      style={{ background: "linear-gradient(160deg, #f4faf9 0%, #eaf5f3 55%, #f0faf8 100%)" }}
    >
      <div className="container-main">
        ...
        <div
          className="cred-featured rounded-3xl p-7 sm:p-10 mb-8 flex flex-col sm:flex-row items-start sm:items-center gap-6"
          style={{
            background: "linear-gradient(135deg, #071520 0%, #0f2233 60%, #0d2e28 100%)",
            border: "1.5px solid rgba(113,187,178,0.25)",
          }}
        >
          {/* Icon */}
          <div
          ...
          >
            <featured.icon className="w-8 h-8 text-mint" />
          </div>
          ...
          <div className="flex-1">
            <div className="flex flex-wrap items-center gap-2 mb-1">
              ...
              <span
                ...
                style={{ background: "rgba(113,187,178,0.15)", color: "#71BBB2", border: "1px solid rgba(113,187,178,0.25)" }}
              >
                Official
              </span>
            </div>
            <p className="text-xs font-body text-mint mb-2">{featured.body}</p>
            <p className="text-sm font-body leading-relaxed mb-3" style={{ color: "rgba(239,233,213,0.5)" }}>
              {featured.desc}
            </p>
            <div
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl"
              style={{ background: "rgba(113,187,178,0.08)", border: "1px solid rgba(113,187,178,0.18)" }}
            >
              <ShieldCheck className="w-3.5 h-3.5 text-mint" />
              <span className="font-body font-semibold text-sm" style={{ color: "#71BBB2" }}>
        ...
        <div className="cred-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-10">
          {credentials.map((c) => (
            <div
              ...
              style={{ border: "1.5px solid rgba(113,187,178,0.12)" }}
            >
              {/* Top row */}
              <div className="flex items-start gap-3 mb-3">
                ...
                <div>
                  <h4 className="font-display font-semibold text-teal text-sm leading-snug">{c.title}</h4>
                  <p className="text-[10px] font-body font-semibold mt-0.5" style={{ color: c.color }}>{c.holder}</p>
                </div>
              </div>

              {/* Divider */}
              <div className="h-px mb-3" style={{ background: "rgba(113,187,178,0.1)" }} />

              {/* Body */}
              <p className="text-[11px] font-body text-teal/50 mb-1.5">{c.body}</p>
              <p className="text-xs font-body text-teal/65 leading-snug">{c.detail}</p>
        ...
        <div className="cred-memberships">
          <p className="text-[10px] font-body font-bold uppercase tracking-widest text-mint text-center mb-5">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {memberships.map((m) => (
              <div
                ...
                style={{ border: "1.5px solid rgba(113,187,178,0.18)" }}
              >
                <span
                  className="w-6 h-6 rounded-full flex items-center justify-center text-[8px] font-body font-black text-white flex-shrink-0"
                  style={{ background: "linear-gradient(135deg, #71BBB2, #3a7a72)" }}
                >
                  {m.short.slice(0, 2)}
                </span>
                <span className="text-xs font-body text-teal/70">{m.name}</span>
              </div>
            ))}
          </div>
        </div>
