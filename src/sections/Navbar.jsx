...
export default function Navbar() {
  ...
  return <>
      <nav
    className={`fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[95%] max-w-6xl transition-all duration-500 ${scrolled ? "glass shadow-glass" : "bg-white/40 backdrop-blur-xl border border-white/30 shadow-lg"} rounded-full px-4 sm:px-6 py-3`}
  >
    ...
    /* Logo */
          ...
          <button onClick={() => handleNav({ href: "#hero" })} className="flex items-center gap-2 group">
            ...
            <span className="font-display font-semibold text-teal text-sm sm:text-base hidden sm:inline">
              Aashirwad
            </span>
          </button>
          ...
          <div className="hidden lg:flex items-center gap-1">
            {navLinks.map((link) => (
              <button
                ...
                className="px-3 py-1.5 text-xs font-body font-medium text-teal/70 hover:text-teal rounded-full hover:bg-teal/5 transition-all duration-300"
              >
                {link.label}
              </button>
            ))}
  ...
  >
              {mobileOpen ? <X className="w-4 h-4 text-teal" /> : <Menu className="w-4 h-4 text-teal" />}
            </button>
          </div>
        </div>
      </nav>
  ...
  >
        <div className="flex flex-col items-center justify-center h-full gap-6">
          {navLinks.map((link, i) => <button
    key={link.href}
    onClick={() => handleNav(link)}
    className="text-2xl font-display font-medium text-teal hover:text-mint transition-colors"
    style={{
