          ...
          <div className="conc-left lg:col-span-2 lg:sticky lg:top-24">
            <div className="relative lg:min-h-[500px]">
              ...
              >
                ...
                <p className="text-sm font-body leading-relaxed mb-8" style={{ color: "rgba(239,233,213,0.6)" }}>
                ...
                <div className="grid grid-cols-3 gap-3 mb-8">
                  {highlights.map((h) => (
                    <div
                      ...
                      style={{ background: "rgba(113,187,178,0.08)", border: "1px solid rgba(113,187,178,0.15)" }}
                    >
                      <div
                        className="font-display font-bold text-xl mb-0.5"
                        style={{ background: "linear-gradient(90deg,#71BBB2,#a8e6df)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}
                      >
                        {h.value}
                      </div>
                      <p className="text-[10px] font-body leading-tight" style={{ color: "rgba(239,233,213,0.5)" }}>
                        {h.label}
                      </p>
                    </div>
                  ))}
                  ...
                  >
                    ...
                    <div className="relative rounded-2xl overflow-hidden mb-5" style={{ aspectRatio: "16/10" }}>
                      ...
                      <div
                        ...
                        style={{ background: "linear-gradient(to top, rgba(7,21,32,0.8) 0%, rgba(7,21,32,0.1) 60%, transparent 100%)" }}
                      />
                      <span
                        className="absolute bottom-3 right-4 font-display font-black text-5xl leading-none select-none"
                        style={{ color: "rgba(113,187,178,0.4)" }}
                      >
                        {s.num}
                      </span>
                    </div>
                    ...
                    <div
                      ...
                      style={{ background: "rgba(113,187,178,0.12)", border: "1px solid rgba(113,187,178,0.25)" }}
                    >
                      <s.icon className="w-3 h-3 text-mint" />
                      <span className="text-[10px] font-body text-mint font-semibold uppercase tracking-widest">
                        Step {s.num}
                      </span>
                    </div>

                    {/* Title */}
                    <h3 className="font-display font-bold text-white text-2xl mb-1.5 leading-tight">
                      {s.title}
                    </h3>

                    {/* Subheading */}
                    <p className="font-body text-sm font-semibold mb-3" style={{ color: "#71BBB2" }}>
                      {s.subheading}
                    </p>

                    {/* Detail */}
                    <p className="font-body text-sm leading-relaxed" style={{ color: "rgba(239,233,213,0.65)" }}>
                      {s.detail}
                    </p>
                  </div>
                ))}
          ...
          <div className="lg:col-span-3">
            {services.map((s, idx) => (
              <div key={s.num}>
                ...
                {/* Service row */}
                <div
                  className={`conc-row group flex items-start gap-4 py-5 transition-all duration-200 -mx-3 px-3 rounded-xl cursor-pointer ${activeIdx === idx ? "bg-white/[0.04]" : "hover:bg-white/[0.02]"}`}
                  style={{
                    borderBottom:
                      idx < services.length - 1 && openMobile !== idx
                        ? "1px solid rgba(113,187,178,0.1)"
                        : "none",
                  }}
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      background: activeIdx === idx ? "rgba(113,187,178,0.22)" : "rgba(113,187,178,0.12)",
                  ...
                  <div className="flex-1 min-w-0">
                    <h3
                      className={`font-display font-semibold text-sm sm:text-base mb-1 transition-colors duration-200 ${activeIdx === idx ? "text-mint" : "text-white group-hover:text-mint"}`}
                    >
                      {s.title}
                    </h3>
                    <p className="text-xs sm:text-sm font-body leading-relaxed" style={{ color: "rgba(239,233,213,0.5)" }}>
                      {s.desc}
                    </p>
                  </div>

                  {/* Step number + mobile chevron */}
                  <div className="flex items-center gap-1.5 flex-shrink-0 mt-1">
                    <span
                      className={`font-display font-black text-3xl leading-none select-none transition-opacity duration-300 ${activeIdx === idx ? "opacity-70" : "opacity-[0.35] group-hover:opacity-[0.65]"}`}
                      style={{ color: "#71BBB2" }}
                    >
                      {s.num}
                    </span>
                    <ChevronDown
                      className={`w-4 h-4 text-mint lg:hidden transition-transform duration-300 ${openMobile === idx ? "rotate-180" : ""}`}
                    />
                  </div>
                </div>

                {/* Mobile accordion — image left, content right */}
                <div
                  className={`lg:hidden overflow-hidden transition-all duration-500 ease-in-out ${openMobile === idx ? "max-h-60 opacity-100" : "max-h-0 opacity-0"}`}
                  style={{
                    borderBottom: idx < services.length - 1 ? "1px solid rgba(113,187,178,0.1)" : "none",
                  }}
                >
                  <div
                    className="flex gap-3 py-3 px-3 mb-1 rounded-xl"
                    style={{ background: "rgba(113,187,178,0.06)", border: "1px solid rgba(113,187,178,0.12)" }}
                  >
                    {/* Image */}
                    ...
                    <div className="flex-1 min-w-0 pt-0.5">
                      <p
                        className="text-[11px] font-body font-semibold mb-1.5 leading-snug"
                        style={{ color: "#71BBB2" }}
                      >
                        {s.subheading}
                      </p>
